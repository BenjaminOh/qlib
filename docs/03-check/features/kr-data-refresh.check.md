...
### ⑦ 데일리 갱신 순서 결함 — instruments 선연장으로 dump no-op (2026-07-28 발견) — 심각도: **높음**

- **증상**: 7/24 저녁부터 `live_signal: empty prediction` 재발. 캘린더는 매일 증가하는데
  피처 bin은 7/22에서 동결(870일 캘린더 vs 866행 bin).
- **원인**: `_fetch_universe`가 fetch 직후 `generate_instruments_files(merge=True)`로 instruments
  end를 오늘로 선연장 → 이후 `dump_update`가 all.txt end와 CSV를 비교해 "이미 최신"으로 판단,
  0행 append. (7/22 수동 백필 때 수동으로 피했던 순서 문제가 데일리 코드에 그대로 내장돼 있었음)
- **수정 (2026-07-28, `e4b0a804`)**: 순서를 **fetch → dump_update → instruments 연장**으로 재배열.
  데이터는 7/23~7/28 재백필, as_of=7/29 신호 생성 확인.

### 부가 기록 (2026-07-28)

- KIS 모의투자 키 재발급 적용 + 신규 모의계좌 `50199531-01`(가상자금 1,000만원) 교체 —
  지난주 잔고 500 에러의 근본원인은 **구 키/계좌 만료**였음. 잔고 조회 정상 확인.
- 7/23~7/24 open 주문 REJECTED는 만료 계좌 탓 — 신규 계좌로 해소.
