...
6. **사용자별 appkey 축** — 2026-08-26 에 `trail_watch`(장중 5분 폴링)가 들어오면서
   ...
   (10인×20종목 = 200종목이면 모의 기준 한 스윕에 240초). 남의 계좌를 내 appkey 로
   거래할 수 없으므로 애초에 사용자마다 자기 appkey 를 쓰는 것이 유일한 형태다.
   새 프로젝트가 다중 사용자를 받는다면 `kis_client._appkey_scope`(redis 게이트 키)와
   `ACCOUNT_STRATEGIES`(계좌↔전략 맵)가 **사용자 차원까지 확장**돼야 한다. 지금 둘 다
   프로세스 전역 상수다.
