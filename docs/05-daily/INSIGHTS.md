...
10. 브래킷 판정(15:48 체인) — exits의 kind(익절/손절/트레일 청산/부분 익절)·
   ...
   **[2026-08-04 확정] 7곡선 실험 매트릭스**: open(순위 이탈) / close·flow(TP+10%/전저점) /
   trail(−7% 트레일) / scale(+7% 절반→트레일) / limit(−3% 지정가 예약 매수, 랭크순 최대 2체결,
   미체결 취소 + TP+10%). limit은 15:2x 슬롯 없음 — 매수도 15:48 체인에서 당일 봉으로 판정.
   점검 항목: limit의 rested/fills/cancelled 수, scale의 부분 익절 1회성(재발동 금지) 확인.
